...
interface ModeToggleProps {
  mode: Settings['mode'];
  isPro: boolean; // This will be replaced by credit system logic in a future story
...
export function ModeToggle({ mode, isPro, onChange, onUpgradePrompt }: ModeToggleProps) {
  ...
  return (
    ...
    >
      ...
      <ToggleGroupItem
        value="ai"
        className="flex items-center space-x-3 px-6 py-4 rounded-xl transition-all duration-200 relative data-[state=on]:bg-gradient-to-r data-[state=on]:from-purple-50 data-[state=on]:to-pink-50 data-[state=on]:border-2 data-[state=on]:border-purple-500 data-[state=on]:text-purple-700 bg-gray-50 border-2 border-gray-200 text-gray-600 hover:bg-gray-100"
        aria-label="AI Mode"
      >
        <div className="text-left">
